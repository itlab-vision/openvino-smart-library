...
class Thread(QThread):
    ...
    def faceRecognition(self):
        args = dict(name = frName, dll = dllPath , db= dbPath)
...
class StartWindow(QtWidgets.QMainWindow, StartWin.Ui_MainWindow):
     ...
     def signIn(self):
        if (self.ID != self.UID):
            role = (self.CSV.GetUser(self.ID))[1]
            self.close()
            if (int(role.role_id) == 1):
                self.readerWin = ReaderWindow(self.ID, self.CSV)
                self.readerWin.show()
            else:
                self.adminWin = AdminWindow(self.ID, self.CSV)
                self.adminWin.show()           
